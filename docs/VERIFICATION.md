...
Sur la branche d'essai 200 Mo non déployée, un import local CSV de **199 040 027 octets / 40 000 lignes** a parcouru diagnostic, mapping, traitement et export avec PostgreSQL local. Le test d'intégration vérifie maintenant qu'un upload signé est diagnostiqué et haché en une seule lecture, et qu'un mapping aux options inchangées ne relit pas l'original. Un autre test vérifie que la lecture S3 respecte la vitesse du consommateur. `npm run format:check`, lint, typage, **58 tests**, build et **9 parcours Playwright** passent. Aucun transfert Backblaze de 200 Mo ni traitement hébergé de ce volume n'est encore qualifié ; la limite déployée reste 50 Mo.
